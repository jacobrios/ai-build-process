...
- **Copy the safety nets from `~/.claude/templates/project-safety-nets/` rather than rebuilding them from a description.** The README there says what each file does, when it fires, and what to adapt per project. Not all of them are hooks: check the README for where each one is wired, and confirm it runs. A file copied but never wired does nothing, and looks installed. Why: a rule describing a hook is not a hook, and rebuilding one from prose reintroduces every edge case the working version already handles. When a project improves one of these in a way that is not project-specific, copy the improvement back to the template.
